...
for(const id of WEAPON_IDS){for(let level=1;level<=3;level++){const r=purchaseWeaponUpgrade(p,id);assert.ok(r.ok);p=r.profile;assert.equal(p.wasteland.weapons.levels[id],level);assert.equal(Object.hasOwn(p,'weapons'),false);}assert.equal(purchaseWeaponUpgrade(p,id).ok,false);}
assert.equal(p.credits,11000);assert.equal(purchaseWeaponUpgrade(createProfile(),'ufo').ok,false);
assert.deepEqual(normalizeProfile(JSON.parse(JSON.stringify(p))).wasteland.weapons,p.wasteland.weapons);
assert.deepEqual(normalizeWeapons({levels:{ufo:99,bomb:-4,crossbow:NaN,star:1.9}}).levels,{ufo:3,bomb:0,crossbow:0,star:1});
const d=new Duel();d.startCampaign({mode:'wasteland',weaponLevels:p.wasteland.weapons.levels});d.state.status='racing';
...
assert.notEqual(bestKey(result),bestKey({...result,weaponLevels:p.wasteland.weapons.levels}));
let board=recordFinish(createLeaderboard(),result,{id:'test',name:'Test'}).board;
board=recordFinish(board,{...result,weaponLevels:p.wasteland.weapons.levels,timeSec:100},{id:'test',name:'Test'}).board;
assert.equal(board.entries.length,2);assert.deepEqual(board.entries[1].weaponLevels,p.wasteland.weapons.levels);
