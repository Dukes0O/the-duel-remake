...
const app=ownTestCourses(new App());let count=0;const check=(value,message)=>{assert(value,message);count++;};
...
app.returnToMenu();check(app.startCampaign({startStage:arena,car:'titan_monster'}),'arena remains available before the recommended vehicle is unlocked');check(app.duel.state.car==='falcone_f42','requesting a locked car still falls back to an owned starter');app.returnToMenu();
...
memory.clear();const mediumApp=ownTestCourses(new App());
...
memory.clear();const stuntApp=ownTestCourses(new App()),stuntIndex=COURSE.findIndex(stage=>stage.stuntTrial);stuntApp.profile.credits=20000;stuntApp._saveProfile();stuntApp.unlockCar('titan_monster');
