...
check(FEATURE_STATES['career-backup'] === 'dev' && FEATURE_STATES.wasteland2 === 'on'
  ...
  && FEATURE_STATES['crash-effects'] === 'dev'
  && FEATURE_STATES['titan-climb'] === 'dev' && FEATURE_STATES['muddy-hollow'] === 'dev'
  && Object.keys(FEATURE_STATES).length === 8,
  'career backup, the Scrapdome, crash physics, crash effects, Titan climbing and Muddy Hollow stay in QA; Wasteland 2 and Hidden Road are released; roadside destruction has no switch');
...
check(!productionFlags.enabled('career-backup') && !productionFlags.enabled('scrapdome') &&
  !productionFlags.enabled('crash-physics') && !productionFlags.enabled('crash-effects') &&
  !productionFlags.enabled('titan-climb') &&
  !productionFlags.enabled('muddy-hollow'),
  'production keeps the QA-only career backup, Scrapdome, crash physics, crash effects, Titan climbing and Muddy Hollow off');
