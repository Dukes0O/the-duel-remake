...
import {createProfile,normalizeProfile,completionCarProgress,isCarUnlocked,getUpgradeLevels,upgradedCar,purchaseUpgrade,UPGRADE_TYPES,UPGRADE_COSTS,CAR_PRICES} from '../src/progression.js';
...
ok(html.includes('aria-label="Fully tuned cars: 0 of 8"'),'Completion progress is accessible');ok(html.includes('Unlocks automatically')&&html.includes('No credit purchase.'),'Garage explains automatic free reward');
ok(!tags(html).some(tag=>tag.includes('data-action="unlock-car"')),'Locked achievement has no cash purchase button');ok(!tags(html).some(tag=>tag.includes('data-upgrade=')),'Locked reward cannot buy upgrades before ownership');
ok(html.includes(`${speedKph(upgradedCar(CARS[reward]).topSpeed)}</b><span>TOP SPEED / km/h`),'Locked preview already shows its factory-max metric speed');ok(html.includes(formatSpeed(CARS[reward].offRoadSpeed)),'Garage offroad target also uses km/h');
