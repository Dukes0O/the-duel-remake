...
try{
  ...
  for(const [startStage,event]of COURSE.entries()){
    ...
    const normalized=normalizeRaceSettings({eventId:event.id,car:'titan_monster'},createProfile());same(normalized.eventId,event.id,'locked-car settings do not lock their course');same(normalized.car,'falcone_f42','locked-car settings cannot grant the recommended car');
  ...
  for(const car of Object.keys(CARS))for(const [startStage,event]of COURSE.entries()){
    ...
    same(app.duel.state.mode,event.stuntTrial||['chase','drift','checkpoint'].includes(event.kind)?'duel':'timetrial','objective-only modes remain enforced without vehicle forcing');
  ...
  for(const [stageIndex,event]of COURSE.entries())for(const car of Object.keys(CARS)){
    ...
    check(isValidFinish(result),'every car with the complete objective evidence is eligible for records');check(!isValidFinish({...result,completed:false}),'open eligibility still rejects incomplete races');
  ...
  const env={app,choices,ui,COURSE,CARS,CPU_REWARDS,UPGRADE_TYPES,bestKey,eventKey,getLeaderboard,getEquippedDriverId,isCarUnlocked,supportsRouteVariants,getRouteVariantForSeed,syncRaceChoiceButtons,
    root:{querySelectorAll:()=>[]},profile:()=>app.profile,text:(id,value)=>{values[id]=String(value);},credits:value=>Math.floor(value||0).toLocaleString(),time:value=>String(value),escapeHTML:value=>String(value),
