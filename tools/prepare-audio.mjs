...
function process(source,target,peak,{start=0,end=Infinity,loop=true,fadeSeconds=.06,highpass=0,rmsTarget=0,levelWindowSeconds=0}={}){let {samples:a,rate}=read(source);
  ...
  if(levelWindowSeconds){
    ...
    for(let i=0;i<n;i++){
      gains[i]=Math.max(.6,Math.min(1.7,reference/Math.sqrt(Math.max(1e-9,power/width))));
...
process('v8-rev-source.wav','engine-idle.wav',.76,{...engineOptions,start:.65,end:1.8,rmsTarget:.15});
process('acceleration-source.wav','engine-load-low.wav',.8,{...engineOptions,start:1.7,end:3.2});
process('acceleration-source.wav','engine-load-mid.wav',.8,{...engineOptions,start:4.3,end:5.7});
...
process('v8-rev-source.wav','engine-coast.wav',.74,{...engineOptions,start:6.2,end:7.6,rmsTarget:.14});
