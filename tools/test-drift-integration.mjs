...
const create=()=>{const app=ownTestCourses(new App());app.profile.credits=20000;app._saveProfile();check(app.unlockCar(stage.requiredCar).ok,'muscle car unlock succeeds');return app;};
const starter=ownTestCourses(new App());check(starter.startCampaign({startStage:index,car:'falcone_f42'}),'a starter car can enter drift trial without the recommended car');check(starter.duel.state.car==='falcone_f42','drift entry preserves the chosen owned vehicle');starter.returnToMenu();
let example;
...
app.returnToMenu();const oldState={...app.duel.state},oldResult={...priorResult};app.addPlayer('Independent drift driver');app._settleResult(oldResult,oldState);check(app.profile.credits===0&&app.profile.history.length===0,'late old-player drift result cannot pay another local player');check(!app.startCampaign({startStage:index}),'new player does not inherit another player’s course access');ownTestCourses(app);check(app.startCampaign({startStage:index,car:'banshee_muscle'})&&app.duel.state.car==='falcone_f42','new player can enter but cannot use another player’s locked Banshee');
