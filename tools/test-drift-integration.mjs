...
check(!new App().startCampaign({startStage:index}),'locked player cannot enter drift trial');
let example;
for(const cpuDifficulty of ['easy','medium','hard']){
  ...
  check(app.startCampaign({startStage:index,car:'falcone_f42',mode:'timetrial',cpuDifficulty,seed:42}),'owned drift event starts');
  const state=app.duel.state;check(state.mode==='duel'&&state.car===stage.requiredCar&&state.seed===1989,'event snapshots required car, objective mode and fixed layout');
...
app.restart();check(app.profile.credits===retryBalance-500&&app.duel.state.drift.bankedScore===0&&app.driftNotice===null,'restart charges one Medium loss and clears chain/notice');app.restart();check(app.profile.credits===retryBalance-500,'restarting the fresh countdown is not another loss');
app.advance(4);const interruptedBalance=app.profile.credits;const recovered=new App();check(recovered.profile.credits===interruptedBalance-500&&!recovered.profile.activeRace,'reload settles one interrupted drift loss');check(new App().profile.credits===interruptedBalance-500,'second reload does not charge again');
app.returnToMenu();const oldState={...app.duel.state},oldResult={...priorResult};app.addPlayer('Independent drift driver');app._settleResult(oldResult,oldState);check(app.profile.credits===0&&app.profile.history.length===0,'late old-player drift result cannot pay another local player');check(!app.startCampaign({startStage:index}),'new local player must earn its own Banshee');
...
for(const invalid of [{won:false},{targetsMet:false},{objectiveMissed:true},{driftScore:3499},{driftScore:Infinity},{driftScore:NaN},{timeSec:151},{completed:false},{abandoned:true},{timeout:true},{laps:1},{car:'falcone_f42'}]){
