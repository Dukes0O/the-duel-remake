...
for (const row of manifest.files) {
  ...
  const staged=execFileSync('git',['show',':public/assets/audio/'+row.file],{maxBuffer:16000000,windowsHide:true});
  assert(bytes.equals(staged),row.file+': staged bytes must not be text-normalized');
...
const catalog=JSON.parse(readFileSync(new URL('./audio/catalog.json',import.meta.url)));
for(const row of catalog.sounds.filter(row=>row.file.startsWith('audio-src/'))){
 const bytes=readFileSync(new URL('../'+row.file,import.meta.url));
 const staged=execFileSync('git',['show',':'+row.file],{maxBuffer:16000000,windowsHide:true});
 assert(bytes.equals(staged),row.file+': source bytes preserved in Git');
 assert.equal(createHash('sha256').update(staged).digest('hex'),row.sha256,row.file+': catalog checksum');
}
