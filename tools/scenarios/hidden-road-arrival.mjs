...
async function runCostRefinement(context,round){
  ...
  for(const quality of ['high','performance']){
    ...
    await context.evaluate(`(()=>{const a=window.__qaApp;a.setGraphicsQuality(${JSON.stringify(quality)});a.startCampaign({mode:'wasteland',startStage:0,seed:1989,car:'falcone_f42',difficulty:'casual'});a.stop();a.audio.setMuted(true);Object.assign(a.duel.state,{status:'racing',paused:false,countdown:0,traffic:[],opponents:[],rival:null});document.querySelectorAll('details').forEach(n=>n.style.display='none');a.onFrame?.(a.duel.state);window.__render.renderFrame();})()`);
...
export async function run(context) {
  ...
  for(const quality of ['high','performance']){
    ...
    await context.evaluate(`(() => {const app=window.__qaApp;if(!Object.getOwnPropertyDescriptor(window,'localStorage')?.value||!window.name.startsWith('__duel_qa_tab_v2:'))throw Error('Memory-only storage is required');const added=app.addPlayer(${JSON.stringify(`Arrival ${quality}`)});if(!added.ok||app.profile.wasteland?.discoveredGate!==false)throw Error('Fresh arrival player is not on a first gate visit');app.setGraphicsQuality(${JSON.stringify(quality)});app.startCampaign({mode:'wasteland',startStage:0,seed:1989,car:'falcone_f42',difficulty:'casual'});app.stop();Object.assign(app.duel.state,{status:'racing',paused:false,countdown:0,traffic:[],opponents:[],rival:null});document.querySelectorAll('details').forEach(n=>n.style.display='none');app.onFrame?.(app.duel.state);window.__render.renderFrame();})()`);
