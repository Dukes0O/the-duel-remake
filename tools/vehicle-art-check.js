...
const labels=Object.fromEntries(Object.entries(CARS).map(([key,car])=>[key,car.name])),models=new Map(),assets=createVehicleAssets();
let vehicle=null,selected='falcone_f42',pose='front',damaged=false,neutral=false,assetMessage='',showRevision=0;
...
async function show(key){const revision=++showRevision;if(vehicle)scene.remove(vehicle);vehicle=null;selected=key;assetMessage='Loading selected model…';
  if(!models.has(key)){const ready=await assets.load(key,{retry:true});if(revision!==showRevision)return;if(!ready){assetMessage='Model unavailable. Select it again to retry.';return;}models.set(key,assets.create(key));}
  if(revision!==showRevision)return;assetMessage='';vehicle=models.get(key);scene.add(vehicle);applyVehiclePaint(vehicle,neutral?{id:'review-neutral',color:0x939d9c,metalness:.3,roughness:.27,clearcoat:.8}:null);damage(damaged);view();for(const button of document.querySelectorAll('#models button'))button.setAttribute('aria-pressed',String(button.dataset.key===key));}
...
for(const [name,callback]of[['Front three-quarter',()=>view('front')],['Rear three-quarter',()=>view('rear')],['Side silhouette',()=>view('side')],['Wheel and nose detail',()=>view('detail')],['Same neutral paint',()=>{neutral=!neutral;show(selected);}],['Toggle damage',()=>damage(!damaged)],['Steer driver',()=>{if(!vehicle)return;updateDriver(vehicle.userData.driver,.8,.2);vehicle.userData.wheelPivots.filter(p=>p.userData.front).forEach(p=>p.rotation.y=.45);}]] ) {const button=document.createElement('button');button.textContent=name;button.onclick=callback;document.querySelector('#views').append(button);}
