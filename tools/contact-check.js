...
function begin(){
  ...
  app.startCampaign({car:'falcone_f42',startStage:0,seed:1989,difficulty:'casual',cpuDifficulty:'medium',mode:'timetrial'});
  app.stop();app.cameraMode='chase';app.lightingMood='clear';app.ghostEnabled=false;app.ghostPose=null;
  const s=app.duel.state;
  Object.assign(s,{status:'racing',paused:false,countdown:0,stageTimeSec:10,totalTimeSec:10,speedMph:0,revs:0,traffic:[],rival:null});
  s.police.pursuit=null;s.input={throttle:0,brake:0,steer:0,boost:false,shiftUp:false,shiftDown:false};
  other=null;selectedCactus=null;return s;
}
...
function settle(){
  ...
  app.stop();app.audio.setPaused(true);app.onFrame?.(s);report();
}
