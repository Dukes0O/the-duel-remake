...
function run(cpuDifficulty, seed = 1989) {
  ...
  const duel = app.duel;
  const fires = [], hits = [], crashes = [], snapshots = [], bins = { aheadFar: 0, aheadNear: 0, behindNear: 0, behindFar: 0 };
  ...
  const result = { cpuDifficulty, seed, status: state.status, completed: state.results?.completed,
    ...
    fires: fires.filter(row => row.type !== 'star'), hits, crashes, snapshots };
  app.dispose();
  return result;
}
...
if (process.argv.includes('--baseline')) {
  ...
  console.log(JSON.stringify({ seed1989: rows.filter(row => row.seed === 1989)
    .map(({ cpuDifficulty, completed, won, opportunities, fires, hits, crashes }) =>
      ({ cpuDifficulty, completed, won, opportunities, fires: fires.length,
        playerHits: hits.filter(hit => hit.victim === 'player').length, crashes })),
