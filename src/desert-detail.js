...
// A cactus can sit between the terrain ribbon's coarse lateral vertices.
// The analytic height there is not always the rendered triangle height,
// especially beside a flattened shortcut bed. Cache only nearby ribbon rows
// and intersect their actual Float32 triangles without allocating a mesh.
const cactusTerrainRows=new WeakMap();
function cactusGroundHeight(course,tree){
  let rows=cactusTerrainRows.get(course.samples);
  if(!rows){rows=new Map();cactusTerrainRows.set(course.samples,rows);}
  const step=course.def.offroad?2:4;
  const row=s=>{
    if(rows.has(s))return rows.get(s);
    // Same natural-route columns and added branch-bed samples as terrainGeometry.
  ...
  const height=(a,b,c)=>{
    const dx=b.x-a.x,dz=b.z-a.z,ex=c.x-a.x,ez=c.z-a.z,px=tree.x-a.x,pz=tree.z-a.z,det=dx*ez-dz*ex;
    if(Math.abs(det)<1e-9)return null;
    const u=(px*ez-pz*ex)/det,v=(dx*pz-dz*px)/det;
    return u>=-1e-7&&v>=-1e-7&&u+v<=1+1e-7?a.y+u*(b.y-a.y)+v*(c.y-a.y):null;
  };
  const start=Math.floor(tree.s/step)*step;
  ...
  return course.groundAt(tree.s,tree.off).y;
}
