...
const rowKey=row=>[row.playerId,row.eventKey,row.car,...(row.mode==='wasteland'?['wasteland',weaponSignature(row)]:[]),row.driverSignature??driverModifierSignature(row.driverId,row.car),rivalSignature({...row,stageIndex:COURSE.findIndex(stage=>stage.id===row.eventId)})].join('|');
...
export function recordFinish(board,result,player){
  ...
  const event=eventKey(result),id=stageEventId(result.stageIndex),key=rowKey({playerId:player.id,eventKey:event,eventId:id,car:result.car,driverId,driverSignature,mode:result.mode,weaponLevels:result.weaponLevels,rival:result.rival});
  ...
  const entry={...(result.mode==='wasteland'?{weaponLevels:normalizeWeapons({levels:result.weaponLevels}).levels}:{}),playerId:player.id,playerName:playerName(player.name),eventKey:event,eventId:id,eventName:COURSE[result.stageIndex].name,layoutVersion:COURSE[result.stageIndex].layoutVersion??1,seed:(result.seed??1989)>>>0,laps:result.laps,car:result.car,timeSec:result.timeSec,
