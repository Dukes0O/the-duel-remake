...
import { createProfile, getUpgradeLevels, isCarUnlocked, upgradedCar, UPGRADE_COSTS, UPGRADE_TYPES, CAR_PRICES } from './progression.js';
...
const choices = { car: app.duel.state.car, difficulty: app.duel.state.difficulty, mode: 'duel', startStage: app.menuStage || 0 };
let rendererPromise, lastScreen, routeCourse, routeMap, mapTick = 0, garageOpen = false, garageCar = choices.car, garageMessage = '';
...
root.innerHTML = `<div id="stage" class="in-menu"><div id="view3d" aria-label="Three-dimensional driving scene"></div><div class="film-grain"></div><div id="overlay">
  ...
  <main id="menu-screen" class="menu-screen">
    ...
    <section class="race-setup" aria-label="Race setup"><div class="setup-line scene-line"><label class="field-label" for="scene-select">CHOOSE A SCENE</label><select id="scene-select" aria-label="Choose a scene">${COURSE.map((scene,index)=>`<option value="${index}">${scene.name}</option>`).join('')}</select></div><div class="setup-line"><span class="field-label">THE CHALLENGE</span><div class="segmented" role="group" aria-label="Race mode"><button class="choice on" data-mode="duel" aria-pressed="true">RIVAL DUEL</button><button class="choice" data-mode="timetrial" aria-pressed="false">TIME TRIAL</button></div></div><div class="setup-line"><span class="field-label">TRANSMISSION</span><div class="segmented" role="group" aria-label="Difficulty">${Object.entries(DIFFICULTY).map(([key,d]) => `<button class="choice ${choices.difficulty === key ? 'on' : ''}" data-difficulty="${key}" aria-pressed="${choices.difficulty === key}">${d.autoShift ? 'ARCADE / AUTO' : 'PRO / MANUAL'}</button>`).join('')}</div></div><button id="start-engine" class="start-button" data-action="start"><span>START ENGINE</span>${arrow}</button><p class="start-note">${COURSE.length} STAGES <span>·</span> TRAFFIC <span>·</span> PURSUITS <span>·</span> FIFTH MAJOR CRASH ENDS THE RUN</p></section>
    <section class="garage" aria-label="Choose your car"><div class="garage-heading"><span class="field-label">CHOOSE YOUR WEAPON</span><span id="garage-count">01 / ${String(Object.keys(CARS).length).padStart(2,'0')}</span></div><div class="car-choices" role="group" aria-label="Car">${Object.entries(CARS).map(([key,c],i) => `<button class="car-choice ${choices.car === key ? 'on' : ''}" data-car="${key}" aria-pressed="${choices.car === key}"><span class="car-index">0${i+1}</span><span class="car-name">${c.name}<small class="car-lock" data-lock="${key}"></small></span><span class="car-marker">↗</span></button>`).join('')}</div><div class="car-specs"><div><b id="car-speed"></b><span>TOP SPEED / MPH</span></div><div><b id="car-gears"></b><span>GEARS</span></div><div><b id="car-character"></b><span>DRIVING CHARACTER</span></div></div><p class="route-preview"><span class="route-symbol">↝</span><span>FIRST UP <b id="scene-name">${COURSE[0].name}</b></span><span id="scene-length" class="route-length">${(COURSE[0].lengthU/1000).toFixed(1)} KM</span></p><button class="garage-tune" data-action="garage">TUNE YOUR CAR <span>→</span></button></section>
...
function updateMenuCar() {
  ...
  for (const button of root.querySelectorAll('[data-car]')) {
    const owned = isCarUnlocked(profile(), button.dataset.car);
    button.classList.toggle('on', button.dataset.car === choices.car);
    button.classList.toggle('locked', !owned); button.setAttribute('aria-pressed', String(button.dataset.car === choices.car));
    button.querySelector('.car-lock').textContent = owned ? '' : 'LOCKED';
  }
}
function updateMenuScene() {
  const stage = COURSE[choices.startStage] || COURSE[0];
  app.menuStage = choices.startStage; ui['scene-select'].value = choices.startStage;
  text('scene-name', stage.name); text('scene-length', `${(stage.lengthU/1000).toFixed(1)} KM`);
...
function openGarage(car = choices.car) {
  ...
  garageCar = car; garageMessage = ''; garageOpen = true; app.menuCar = car; lastScreen = null; renderState(app.duel.state);
}
...
root.addEventListener('click',e => {
  ...
  switch (button.dataset.action) {
    case 'start': garageOpen = false; app.startCampaign(choices); break;
    case 'garage': openGarage(); return;
    case 'garage-close': closeGarage(); return;
    case 'unlock-car': { const result = app.unlockCar(garageCar); if(result.ok) choices.car = garageCar; refreshGarage(result.ok ? `${CARS[garageCar].name} is yours. Ready for the next race.` : result.reason); return; }
    case 'garage-select': choices.car = garageCar; closeGarage(); return;
    ...
    case 'menu': garageOpen = false; app.returnToMenu(); updateMenuCar(); updateMenuScene(); break;
...
function garageScreen() {
  ...
  return `<section class="garage-panel" role="dialog" aria-modal="true" aria-labelledby="garage-title"><header class="shop-heading"><div><p class="eyebrow"><i></i> THE GARAGE</p><h2 id="garage-title">MAKE IT YOURS.</h2></div><div class="shop-wallet"><span>YOUR CREDITS</span><b>${credits(saved.credits)}<small> CR</small></b></div><button class="shop-close" data-action="garage-close" aria-label="Close garage">×</button></header><nav class="shop-cars" aria-label="Garage cars">${Object.entries(CARS).map(([key,item])=>`<button data-garage-car="${key}" class="${key===garageCar?'on':''}" aria-pressed="${key===garageCar}"><i style="--car-color:#${item.color.toString(16).padStart(6,'0')}"></i><span>${item.name}</span><small>${isCarUnlocked(saved,key)?'OWNED':`${credits(CAR_PRICES[key])} CR`}</small></button>`).join('')}</nav><div class="shop-summary"><div><span class="field-label">${owned?'YOUR BUILD':'UNLOCK THE NEXT LEVEL'}</span><h3>${car.name}</h3></div><div><b>${Math.round(tuned.topSpeed)}</b><span>TOP SPEED / MPH</span></div><div><b>${tuned.gears.length}</b><span>GEARS</span></div></div>${owned?`<div class="upgrade-grid">${cards}</div>`:`<div class="unlock-card"><div><p class="eyebrow">CARBON GT PACKAGE</p><h3>Built to chase<br>something faster.</h3><p>Gold wheels. Carbon aero. More power and grip.</p></div><div class="unlock-price"><b>${credits(price)}<small> CR</small></b><button class="start-button" data-action="unlock-car" ${saved.credits<price?'disabled':''}><span>UNLOCK CAR</span>${arrow}</button><p>${saved.credits<price?`Earn ${credits(price-saved.credits)} more credits to unlock.`:'Ready for your garage.'}</p></div></div>`}<p class="garage-message" role="status" aria-live="polite">${garageMessage||'Win courses to earn credits. Upgrades apply from your next race.'}</p><footer class="shop-footer"><p>${app.profileSaved===false?'Progress is saved for this session only.':'Progress saves in this browser.'}</p>${owned?action('DRIVE THIS CAR','garage-select',true):action('BACK TO THE ROAD','garage-close')}</footer></section>`;
}
...
function renderState(s) {
  ui.overlay.dataset.credits = String(profile().credits);
  text('menu-credits', `${credits(profile().credits)} CR`);
  ...
  const screen=`${s.status}:${!!s.paused}:${showImpact}:${garageOpen}:${garageOpen ? garageCar + ':' + profile().credits : ''}`;
  if (screen!==lastScreen) {
    ...
    const modal=menu && garageOpen ? garageScreen() : menu||showImpact?'':modalScreen(s); ui['modal-layer'].innerHTML=modal; ui['modal-layer'].hidden=!modal; ui.stage.classList.toggle('has-modal',!!modal); ui['countdown'].hidden=s.status!=='countdown'||!!s.paused;
...
function updateHud(s) {
  ...
  const callout=crash?(s.catastrophic?'CATASTROPHIC IMPACT':s.lastCrashReason==='engine_blew'?'ENGINE BLOWN':s.lastCrashReason==='rock'?'ROCK IMPACT':'COLLISION'):s.calloutTimer>0?s.callout:s.boundaryWarning?'RETURN TO THE ROAD':s.offRoad?'LOOSE SURFACE':s.drifting?'DRIFT':'';
  ui['race-callout'].hidden=!callout||!!s.paused; text('callout-kicker',s.catastrophic?'FIVE HITS. END OF THE ROAD.':crash?hits===DRIVE.majorCrashLimit-1?'CHASSIS CRITICAL. MAKE THIS LIFE COUNT.':'SHAKE IT OFF. KEEP DRIVING.':s.boundaryWarning?'COURSE BOUNDARY · RESET AHEAD':s.offRoad?'FIND THE TARMAC':s.boosting?'FULL SEND':'MAKE EVERY MOVE COUNT');text('callout-text',callout);ui['race-callout'].classList.toggle('crash-callout',crash);
...
document.addEventListener('keydown',e=>{if(e.code==='Escape'&&garageOpen){e.preventDefault();closeGarage();return;}if(e.code!=='Tab'||ui['modal-layer'].hidden)return;const buttons=[...ui['modal-layer'].querySelectorAll('button:not(:disabled),select')],first=buttons[0],last=buttons.at(-1);if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus();}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus();}});
app.onFrame=renderState;updateMenuCar();updateMenuScene();renderState(app.duel.state);ensureRenderer();app.start();
