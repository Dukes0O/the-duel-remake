...
import { createProfile, getUpgradeLevels, isCarUnlocked, upgradedCar, UPGRADE_COSTS, UPGRADE_TYPES, CAR_PRICES, CPU_REWARDS, bestKey, eventKey, milestoneProgress } from './progression.js';
...
const boardFilter={stage:choices.startStage,car:''};
...
root.innerHTML = `<div id="stage" class="in-menu"><div id="view3d" aria-label="Three-dimensional driving scene"></div><div class="film-grain"></div><div id="overlay">
  ...
  <main id="menu-screen" class="menu-screen">
    ...
    <footer class="menu-footer"><span class="footer-label">BUILT FOR THE DRIVE</span><div class="controls-strip"><span title="Hold S / Down / LT at rest to reverse. W / Up / RT brakes reverse and returns to first gear."><kbd>W A S D</kbd> DRIVE / REVERSE</span><span><kbd>SPACE</kbd> BOOST</span><span><kbd>C</kbd> CAMERA</span><span><kbd>Q / E</kbd> MANUAL SHIFT</span><span><kbd>ESC</kbd> PAUSE</span></div><a class="build-label audio-credits" href="/assets/audio/credits.html" target="_blank" rel="noopener">ASSET CREDITS ↗</a></footer>
  </main>
...
function ensureRenderer() {
  if (!rendererPromise) {
    ...
    rendererPromise = import('./render3d.js').then(({attachRenderer}) => { if(uiDisposed)return;rendererHandle=attachRenderer(ui.view3d,app); ui.view3d.classList.add('live'); ui['start-engine'].disabled = false; }).catch(error => {
    ...
    }).finally(() => { if(!uiDisposed)ui['renderer-loading'].hidden = true; });
  }
  return rendererPromise;
}
function updateMenuCar() {
  if (!isCarUnlocked(profile(), choices.car)) choices.car = 'falcone_f42';
  const car = upgradedCar(CARS[choices.car], getUpgradeLevels(profile(), choices.car));
  app.duel.state.car = choices.car; app.menuCar = choices.car;
  text('car-speed', Math.round(car.topSpeed)); text('car-gears', String(car.gears.length).padStart(2,'0'));
  ...
  for(const option of ui['car-select'].options){const key=option.value;option.textContent=isCarUnlocked(profile(),key)?CARS[key].name:`${CARS[key].name} · LOCKED · ${credits(CAR_PRICES[key])} CR`;}
...
function updateMenuScene() {
  ...
  for(const option of ui['scene-select'].options){const item=COURSE[Number(option.value)],locked=item.requiredCar&&!isCarUnlocked(profile(),item.requiredCar);option.disabled=!!locked;option.textContent=`${item.name}${locked?` · UNLOCK ${CARS[item.requiredCar].name.toUpperCase()}`:''}`;}
...
root.addEventListener('click',e => {
  ...
  if(button.dataset.challenge!=null){choices.startStage=Number(button.dataset.challenge);choices.car=COURSE[choices.startStage].requiredCar;updateMenuScene();closeGarage();return;}
  ...
  switch (button.dataset.action) {
    ...
    case 'leaderboard':leaderboardOpen=true;playersOpen=garageOpen=false;boardFilter.stage=choices.startStage;lastScreen=null;break;
    ...
    case 'unlock-car': { const result = app.unlockCar(garageCar); if(result.ok) {choices.car = garageCar;updateMenuScene();}refreshGarage(result.ok ? `${CARS[garageCar].name} is yours.${COURSE.some(event=>event.requiredCar===garageCar)?' A new challenge is now unlocked.':' Ready for the next race.'}` : result.reason); return; }
    ...
    case 'garage-select': choices.car = garageCar;if(COURSE[choices.startStage]?.requiredCar&&COURSE[choices.startStage].requiredCar!==garageCar)choices.startStage=0;updateMenuScene();closeGarage(); return;
...
function leaderboardScreen(){
  ...
  const rows=getLeaderboard(app.leaderboard,{event,car:boardFilter.car});
...
function garageScreen() {
  ...
  const levels = getUpgradeLevels(saved, garageCar), tuned = upgradedCar(car, levels);
  const price = CAR_PRICES[garageCar] || 0;
  ...
  const capabilities=[['ACCELERATION',tuned.accel/maximumAcceleration],['ROAD GRIP',tuned.grip/1.2],['OFFROAD PACE',(tuned.offRoadSpeed??68)/125,`${Math.round(tuned.offRoadSpeed??68)} MPH`],['BRAKING',tuned.braking/maximumBraking]];
  const capabilityStats=`<div class="car-capabilities" aria-label="Car capabilities">${capabilities.map(([label,value,display])=>`<div><span>${label}</span><b>${display||`${(clamp(value)*10).toFixed(1)}<small> / 10</small>`}</b><i aria-hidden="true"><em style="width:${clamp(value)*100}%"></em></i></div>`).join('')}</div><p class="capability-note">Ratings use the same scale for every car. ${owned?'Your installed upgrades are included.':'Compare this car before unlocking it.'} Offroad pace is its loose-surface target.</p>`;

  const pitch={falcone_heritage:['HERITAGE EDITION','The earlier Falcone returns.','The previous F42 body with the same five-speed handling. A separate unlock alongside the redesigned starter.'],aurora_gt:['CARBON GT','Precision at speed.','Carbon aero, gold wheels and balanced grip.'],dusthawk_rally:['RALLY BUILT','Take the rough line.','Rally tires and dirt grip. Unlock Ridge Rally and Timberline Checkpoint Rush.'],banshee_muscle:['BIG MUSCLE','Make every straight count.','Muscle for Midnight Chase and the Neon Drift Trial.'],viper_proto:['TRACK PROTOTYPE','Chase the lap record.','Light weight, sharp braking and fast aero.'],titan_monster:['MONSTER CLASS','Unlock the arena.','Giant tires, crushing weight and entry to Titan Monster Arena.']}[garageCar]||['YOUR CAR','Make it yours.','Tune seven systems to suit the way you drive.'];
...
function updateHud(s) {
  ...
  if(drift){
    ...
    text('drift-notice',notice&&notice.expiresAt>s.stageTimeSec?notice.type==='banked'?`Banked +${credits(notice.points)}`:`Chain lost · ${reason[notice.reason]||'recovery'}`:banked>=target?'Target met. Finish both laps.':chain?'Straighten to bank. Avoid impacts and dirt.':'Slide above 45 MPH to build a chain.');
  }
  ...
  text('damage-label',persistent?`IMPACTS ADD +${app.duel.stageDef.chaseCrashPenaltySec||8} SEC`:hits===DRIVE.majorCrashLimit-1?'CRITICAL · NEXT MAJOR CRASH IS FATAL':`${hits} / ${DRIVE.majorCrashLimit} MAJOR CRASHES`);ui['damage-label'].classList.toggle('critical',!persistent&&hits>=DRIVE.majorCrashLimit-1);
