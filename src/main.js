...
const combatHelp=document.createElement('p');combatHelp.className='combat-help';combatHelp.textContent='MAD MAX DUEL: 1 UFO swap · 2 eight-way bombs · 3 crossbow · 4 invincible star (5s). Weapons recharge. The rival fights back. Wrecks recover; finish first.';root.querySelector('.race-setup').append(combatHelp);
...
function renderState(s) {
  ...
  if(combat){for(const button of weaponHud.querySelectorAll('[data-weapon]')){const key=button.dataset.weapon,left=combat.cooldowns[key];button.disabled=s.status!=='racing'||s.paused||left>0;button.textContent=`${WEAPONS[key].key} · ${WEAPONS[key].name} · ${left>0?Math.ceil(left)+'s':'READY'}`;}weaponStatus.textContent=combat.shield>0?`INVINCIBLE · ${combat.shield.toFixed(1)}s`:`ARMORED DUEL · ${combat.hits} HITS · CPU WEAPONS ACTIVE`;}
