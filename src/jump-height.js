...
export function createJumpHeightReadout(){
  ...
  let airborne=false,peak=0,landedAt=null;
  const clear=()=>{airborne=false;peak=0;landedAt=null;};
  return {update(state,course){
    ...
    if(state.airborne){
      ...
      return {phase:'airborne',heightMeters:height,peakMeters:peak};
    }
    if(airborne){airborne=false;landedAt=time;}
    if(landedAt!==null&&time-landedAt<LANDING_HOLD_SECONDS-1e-9)return {phase:'landed',heightMeters:0,peakMeters:peak};
    clear();return hidden();
  }};
}
