...
export function createChickens(course){
  ...
  const birds=[],collectedAt=new Map(),o=new THREE.Object3D();let lastTime=-Infinity;
  ...
  for(const flock of course.features.flocks){
    ...
    for(let j=0;j<flock.count;j++){
      ...
      birds.push({flock,ds:Math.cos(angle)*radius,off:Math.sin(angle)*radius,
  ...
  function hide(index){
    for(const m of [body,heads,tails,combs,wattles,beaks])set(m,index,0,0,0,0,0,0);
    for(const m of [wings,legs,eyes,feet])for(let j=0;j<2;j++)set(m,index*2+j,0,0,0,0,0,0);
  }
  function update(state,t){
    ...
    birds.forEach((bird,i)=>{
      const flock=bird.flock,age=collectedAt.has(flock.id)?Math.max(0,t-collectedAt.get(flock.id)):-1;
      if(age>3.6){hide(i);return;}
      const nearby=state.status==='racing'&&Math.abs((state.s??0)-flock.s)<26&&state.speedMph>20;
      ...
      const fade=age>2.85?Math.max(0,(3.6-age)/.75):1,k=bird.scale*fade;
