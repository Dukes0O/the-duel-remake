...
export function buildRouteMapGeometry(course,width=400,height=280,padding=28,{discovery}={}){
  ...
  const revealed=hiddenRoadHints(discovery).showPath&&course.hiddenRoad;
  const hiddenSamples=revealed?course.hiddenRoad.samples:[];
  const all=[...course.samples,...hiddenSamples,...branches.flatMap(branch=>branch.world),...practice.map(feature=>feature.world)];let minX=Infinity,maxX=-Infinity,minZ=Infinity,maxZ=-Infinity;
  ...
  if(revealed)geometry.hiddenRoad={points:project(hiddenSamples),gate:projectRoutePoint(geometry,hiddenSamples.at(-1))};
...
export class RouteMap {
  constructor(canvas,{getDiscovery=()=>null}={}){this.getDiscovery=getDiscovery;this.discoveryKey=null;this.canvas=canvas;this.context=canvas.getContext('2d');this.cache=new WeakMap();this.course=null;this.current=null;this.lastDraw=-Infinity;this.pose={};this.lastLabel='';}
  _prepare(course,discovery){
    const discoveryKey=hiddenRoadMapKey(discovery);
    let cached=this.cache.get(course);if(cached&&cached.discoveryKey===discoveryKey&&cached.width===this.canvas.width&&cached.height===this.canvas.height)return cached;
    const geometry=buildRouteMapGeometry(course,this.canvas.width,this.canvas.height,28,{discovery});if(!geometry)return null;
    ...
    drawPracticeMarkers(ctx,geometry);drawHiddenRoadPath(ctx,geometry);
    cached={...geometry,discoveryKey,routePath:route,sectionPaths:sections,branchPaths:branches,base};this.cache.set(course,cached);return cached;
  }
  update(course,state,now=performance.now()){
    if(!this.context||!course?.samples?.length)return;
    const discovery=this.getDiscovery(),discoveryKey=hiddenRoadMapKey(discovery);
    const changed=discoveryKey!==this.discoveryKey||course!==this.course||this.current?.width!==this.canvas.width||this.current?.height!==this.canvas.height;
    if(!changed&&now-this.lastDraw<60)return;
    this.lastDraw=now;if(changed){this.course=course;this.discoveryKey=discoveryKey;this.current=this._prepare(course,discovery);}
    ...
    const opponents=state.opponents|| (state.rival?[state.rival]:[]);
    const opponentColors=['#f6d768','#88d4ca','#ef9f75'];
    opponents.forEach((opponent,index)=>this._actor(course,map,opponent,opponentColors[index%opponentColors.length],8,false));
    ...
    const label=course.def.practice?`${course.def.name} quarry practice map. Untimed exploration, with no laps or finish line. Your arrow shows your heading. ${practiceMapLegendText()}`:`${course.def.name} circuit map. Lap ${lap} of ${course.def.laps||2}. ${section}. Your arrow shows your heading.${opponents.length>1?` Colored markers show ${opponents.length} opponents.`:state.rival?' Yellow is your rival.':''}${police?.active?' Blue and red is the police patrol.':''}${map.branches.length?` Dashed shortcuts ${map.branches.map(branch=>branch.label).join(' and ')}.${branchIndex>=0?` You are on shortcut ${map.branches[branchIndex].label}.`:''}`:''}${state.checkpointRush?` ${state.checkpointRush.passed} of ${state.checkpointRush.total} gates passed. ${state.checkpointRush.nextGate<state.checkpointRush.total?'Gold marks the next gate.':'Finish both laps.'}`:''} Checkered line marks the finish.`;
    const accessibleLabel=label+(map.hiddenRoad?' Dotted road leads to the Rustwall.':'');
    if(accessibleLabel!==this.lastLabel){this.canvas.setAttribute('aria-label',accessibleLabel);this.lastLabel=accessibleLabel;}
    this.canvas.dataset.hiddenRoad=String(!!map.hiddenRoad);this.canvas.dataset.section=section;this.canvas.dataset.lap=String(lap);this.canvas.dataset.shortcuts=String(map.branches.length);
  }
...
export function drawHiddenRoadPath(ctx,map){
  if(!map.hiddenRoad)return;
  ctx.save();ctx.strokeStyle='#dfb674';ctx.lineWidth=2.5;ctx.lineCap='round';ctx.setLineDash([1,5]);ctx.beginPath();
  const points=map.hiddenRoad.points;for(let i=0;i<points.length;i+=2)i?ctx.lineTo(points[i],points[i+1]):ctx.moveTo(points[i],points[i+1]);ctx.stroke();ctx.setLineDash([]);
  const gate=map.hiddenRoad.gate;ctx.fillStyle='#f1d5a1';ctx.beginPath();ctx.moveTo(gate.x,gate.y-4);ctx.lineTo(gate.x+4,gate.y);ctx.lineTo(gate.x,gate.y+4);ctx.lineTo(gate.x-4,gate.y);ctx.closePath();ctx.fill();
  ctx.font='10px Segoe UI, sans-serif';ctx.textAlign=gate.x>map.width/2?'right':'left';ctx.fillText('RUSTWALL',gate.x,gate.y-9);ctx.restore();
}
