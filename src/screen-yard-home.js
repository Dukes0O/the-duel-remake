...
export function yardHomeScreen({profile,playerName,escapeHTML,panel='home',armoryMarkup='',message=''}) {
  ...
  const nav=`<nav class="yard-home-nav" aria-label="Wasteland home">${button('CAREER','yard-career')}${button('TERRITORY MAP','yard-territory')}${button('ARMORY','yard-armory')}${button('CREW','yard-crew')}</nav>`;
  ...
  const content=panel==='territory'?territoryPanel(profile):panel==='crew'?crewPanel(profile,escapeHTML):
    panel==='armory'?armoryMarkup:panel==='career'?careerContent:
      `<div class="yard-home-intro">${intro}<p>Warlord fights and the arena are coming later.</p></div>`;
