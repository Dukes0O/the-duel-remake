...
import {hiddenRoadPresentation} from './hidden-road-ui.js';
...
export function attachRenderer(host, app) {
  ...
  function frame(now = performance.now(),measure=false) {
    ...
    const journeyView=hiddenRoadPresentation(st,course);
    ...
    if (!ready || menu || st.onFoot || journeyView.camera) camera.position.copy(camTarget);
    else {
    ...
    if(!menu&&!journeyView.camera)constrainTunnelCamera(course,camera.position,
