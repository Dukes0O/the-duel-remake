...
function normalizeRegistry(value){
  ...
  const seenIds=new Set(),seenNames=new Set(),players=[];
  for(const p of value.players){const name=playerName(p?.name),id=String(p?.id||'');if(!name||!/^[\w-]{1,80}$/.test(id)||seenIds.has(id)||seenNames.has(name.toLowerCase()))continue;seenIds.add(id);seenNames.add(name.toLowerCase());players.push({id,name,profile:normalizeProfile(p.profile)});}
  if(!players.length)return null;return {version:2,activePlayerId:players.some(p=>p.id===value.activePlayerId)?value.activePlayerId:players[0].id,players};
}
export function loadPlayers(storage){
  let target;try{target=storage??globalThis.localStorage;}catch{return createPlayerRegistry();}
  try{const raw=target?.getItem(PLAYERS_KEY);if(raw){const value=normalizeRegistry(JSON.parse(raw));if(value)return value;}}catch{}
  try{const raw=target?.getItem(PROFILE_KEY);return createPlayerRegistry(raw?normalizeProfile(JSON.parse(raw)):createProfile());}catch{return createPlayerRegistry();}
}
export function savePlayers(registry,storage){try{const target=storage??globalThis.localStorage,normalized=normalizeRegistry(registry);if(!target||!normalized)return false;target.setItem(PLAYERS_KEY,JSON.stringify(normalized));return true;}catch{return false;}}
export function activePlayer(registry){return registry.players.find(p=>p.id===registry.activePlayerId)||registry.players[0];}
export function createPlayer(registry,name){
  name=playerName(name);if(!name)return {registry,ok:false,reason:'Enter a player name.'};
  if(registry.players.some(p=>p.name.toLowerCase()===name.toLowerCase()))return {registry,ok:false,reason:'That player already exists. Select them instead.'};
  const player={id:newId(),name,profile:createProfile()};return {registry:{...registry,activePlayerId:player.id,players:[...registry.players,player]},ok:true,player};
}
export function selectPlayer(registry,id){return registry.players.some(p=>p.id===id)?{...registry,activePlayerId:id}:registry;}
export function replacePlayerProfile(registry,id,profile){return {...registry,players:registry.players.map(p=>p.id===id?{...p,profile:normalizeProfile(profile)}:p)};}
...
export function milestoneProgress(profile){
  ...
  return Object.entries(DRIVING_MILESTONES).map(([id,milestone])=>{const earned=profile?.milestones?.includes(id)||false,total=id==='circuit_tour'?circuits.length:1,progress=earned?total:id==='circuit_tour'?circuits.filter(event=>profile?.circuitWins?.includes(event)).length:0;return {id,...milestone,earned,progress,total};});
}
