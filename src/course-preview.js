import {hiddenRoadMapKey} from './hidden-road-hints.js';
import {buildRouteMapGeometry,drawPracticeMarkers,drawHiddenRoadPath,practiceMapLegendText} from './route-map.js';
...
export function buildCoursePreview(course,width=320,height=200,discovery=null){
  const map=buildRouteMapGeometry(course,width,height,19,{discovery});if(!map)return null;
...
export class CoursePreview {
  ...
  update(course,label='',discovery=null){
    const discoveryKey=hiddenRoadMapKey(discovery);
    if(!this.canvas||!course)return null;
    let data=this.cache.get(course);if(!data||data.discoveryKey!==discoveryKey||data.map.width!==this.canvas.width||data.map.height!==this.canvas.height){data=buildCoursePreview(course,this.canvas.width,this.canvas.height,discovery);if(!data)return null;data.discoveryKey=discoveryKey;this.cache.set(course,data);}
    ...
    if(ctx){
      ...
      drawPracticeMarkers(ctx,map);drawHiddenRoadPath(ctx,map);
    }
