...
The current game has nine two-lap events, a local career and leaderboard, eight selectable cars, recorded engine/tire audio, solid scenery, collision-aware rivals and police, drifting, airborne arena jumps and crushable salvage cars. Timberline Checkpoint Rush has passed input-driven and browser checks. Courses share their road, terrain and collider data with the renderer. Mountain and rally geometry have dedicated mesh-clearance audits. See `SESSION_HANDOFF.md` for the current working state and `VERIFICATION.md` for checked results and any skipped campaign matrices. `ARCHITECTURE.md` describes the scene-module boundaries and the next graphics-work plan.
