...
Race setup shows **Best ghost** only when the selected player, route, car, and settings have a recording. The label includes its finishing time and the number of installed upgrades in its recorded build. Hovering over the label shows the individual upgrade levels. Playback starts enabled; its on/off setting is saved per local player. Turning playback off still records eligible personal bests.

The key uses the named local player ID and the existing personal-best key: course ID, `layoutVersion`, seed, laps, car, Time Trial mode, transmission, CPU difficulty and active driver modifier signature. Neutral and off-specialty drivers keep the original keys; enhanced driver performances remain separate. Upgrades can improve the same car record, so the saved build is shown explicitly. Increment a course's `layoutVersion` when its route geometry or distance changes. Valid old-layout recordings move into `archivedRecords` in the same save; their keys and samples remain intact. They cannot be selected or played against a newer layout. These records stay on this computer; no account, network service, or online leaderboard is involved.
...
`app.ghostPose` is null or a reusable object with `s`, `lateral`, `headingError`, `airHeight`, `speedMph`, optional `groundHeight`, `terrainPitch`, `terrainRoll`, boolean `tumble`, `car`, `playerId`, `timeSec`, `recordTimeSec`, and `upgrades`. Its `headingError` already includes the recorded slip and crash-spin yaw. The renderer must not add the current player's yaw or feed this object into `Duel`. `app.ghostRecord` holds the current race's comparison recording. `app.ghostStatus` is `none`, `recording`, `playing`, `off`, or `finished`.

## Validation

New off-road recordings can append four integer columns to the original seven: absolute ground height in centimetres, physical pitch and roll in 1/10,000 radians, and presence/tumble flags. Optional height is bounded to ±10,000 m and each angle to ±80 radians. Legacy samples retain the original seven-column format and produce null terrain fields with no tumble. Playback interpolates terrain support and preserves continuous tumble angles; recovery intervals still hold then snap. The renderer uses the recorded physical pose, not the live player's position or the road beneath a mountain.

Rally and monster vehicles may roam beyond the earlier 180 m lateral boundary. Their sample validation permits finite travel bounded by recording duration and the existing 500 mph recorded-speed ceiling. Ordinary road vehicles keep their prior strict bounds. All recordings still require the validated lap finish and retain sample/time/storage limits. Titan Freestyle Playground is excluded from keys, recording, storage normalization and playback.

`node tools/test-ghost-terrain.mjs` checks legacy defaults, terrain interpolation, full tumbles, recovery snaps, quantization bounds, capable-car roaming, serialization and retained archives.
