...
Latest: the Harbor Route C checkpoint-edge fix is `index-WMtcm23o.js`, with renderer `render3d-CwwMA6Gg.js` and build ID `20260920145756-c083e8f5bccd`. Its footer reads **26.09.20 14:57 UTC · c083e8**. The live HTTP response and manifest match the built files. The production build and all 100 test suites pass: 494 final core checks including the 73 longer simulations (338.227 seconds), plus 99 other suites (285.15 seconds). Focused race-integrity coverage includes 601 checks. No campaign block was skipped. See `VERIFICATION.md` for the initial test-classification failure, its correction and the plain-language explanation of the longer tests, and `PERFORMANCE_PASS.md` for the earlier performance measurements.

The live browser tab was not refreshed, inspected or controlled for this follow-up. The checkpoint checks use headless simulation and temporary memory-only saves. Refresh the existing game tab from its menu when ready, or use its menu-only update notice if available. Saved accounts, balances and settings are unchanged by this release. No new QA server, browser tab or disposable build was needed; the previous temporary port5175 server and `.qa-dist` were already removed.
