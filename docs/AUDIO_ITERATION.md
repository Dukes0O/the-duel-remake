...
Legacy Freesound downloads are cached outside Git in `C:/Users/kyleb/dev/audio-library/legacy/`; newly selected sources use `C:/Users/kyleb/dev/audio-library/freesound/`. Two licensed originals without a verified direct URL are retained as FLAC in `audio-src/library/`. Rebuild original engine, tire, and ambience assets with `npm run assets:audio`; rebuild new combat assets with `node tools/audio/build-combat.mjs` and the kept voice with `node tools/audio/build-gatekeeper.mjs`. These recipes need installed FFmpeg and their verified source inputs. A normal local rebuild makes no network request, but a clean checkout without the external source cache cannot rebuild every selected sound. The old claim that committed decoded WAVs make FFmpeg unnecessary is obsolete. Fourteen converted runtime files, including one-shot accents, shrank from 6,108,572 WAV bytes to 3,575,193 FLAC bytes without changing decoded signed-16 PCM or loop sample counts.
